...
function createDom(vdom) {
  const dom =
    vdom.type === 'TEXT_ELEMENT' ? document.createTextNode('') : document.createElement(vdom.type);
...
function useState(initialValue) {
  ...
  if (fiber.hooks[index] === undefined) {
    fiber.hooks[index] = typeof initialValue === 'function' ? initialValue() : initialValue;
  }

  const setState = (newValue) => {
    const val = typeof newValue === 'function' ? newValue(fiber.hooks[index]) : newValue;
