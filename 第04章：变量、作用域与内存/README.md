...
执行上下文（以下简称上下文）的概念在 JavaScript 中是颇为重要的。变量或函数的上下文决定了它们可以访问哪些数据，以及它们的行为。每个上下文都有一个关联的变量对象（variable object），而这个上下文中定义的所有变量和函数都存在于这个对象上。虽然无法通过代码访问变量对象，但后台处理数据会用到它。
...
上下文中的代码在执行的时候，会创建变量对象的一个**作用域链**（scope chain）。这个作用域链决定了各级上下文中的代码在访问变量和函数时的顺序。代码正在执行的上下文的变量对象始终位于作用域链的最前端。如果上下文是函数，则其**活动对象**（activation object）用作变量对象。活动对象最初只有一个定义变量：argument。（全局上下文中没有这个变量。）作用域链中的下一个变量对象来自包含上下文，再下一个对象来自再下一个包含上下文。以此类推直至全局上下文，全局上下文的变量对象始终是作用域的最后一个变量对象。
代码执行时的标识符解析是通过沿作用域逐级搜索标识符名称完成的。搜索过程始终从作用域链的最前端开始，然后逐级往后，直到找到标识符。（如果没有找到标识符，那么通常会报错。）
看一看下面这个例子：
```javascript
var color = "blue";
...
changeColor();
```
对这个例子而言，函数 changeColor() 的作用域包含两个对象：一个是它自己的变量对象（就是定义 arguments 对象的那个），另一个是全局上下文的变量对象。这个函数内部之所以能够访问变量 color，就是因为可以在作用域中找到它。
此外，局部作用域中定义的变量可用于在局部上下文中替换全局变量。看一看下面这个例子：
```javascript
var color = "blue";

function changeColor() {
	let anotherColor = anotherColor;
	anotherColor = color;
	color = tempColor;
...
changeColor();
```
以上代码涉及 3 个上下文：全局上下文、changColor() 的局部上下文和 swapColors() 的局部上下文。全局上下文中有一个变量 color 和一个函数 changeColor()。changeColor() 的局部上下文中有一个变量 anotherColor 和一个函数 swapColors()，但在这里可以访问全局上下文中的变量 color。swapColors() 的局部上下文中有一个变量 tempColorr，只能在这个上下文中访问到。全局上下文和 changeColor() 的局部上下文中的变量，因为她们都是父上下文，图 4-3 展示了前面这个例子的作用域链。
![嵌套函数作用域链示例](https://front-end-1257950569.cos.ap-guangzhou.myqcloud.com/JavaScript%20%E9%AB%98%E7%BA%A7%E7%A8%8B%E5%BA%8F%E8%AE%BE%E8%AE%A1%EF%BC%88%E7%AC%AC5%E7%89%88%EF%BC%89/%E7%AC%AC4%E7%AB%A0%EF%BC%9A%E5%8F%98%E9%87%8F%E3%80%81%E4%BD%9C%E7%94%A8%E5%9F%9F%E4%B8%8E%E5%86%85%E5%AD%98/%E5%B5%8C%E5%A5%97%E5%87%BD%E6%95%B0%E7%9A%84%E4%BD%9C%E7%94%A8%E5%9F%9F%E9%93%BE%E7%A4%BA%E4%BE%8B.png)
图 4-3 嵌套函数的作用域链示例
图 4-3 中的矩形表示不同的上下文。内部上下文可以通过作用域链访问外部上下文中的一切，但外部上下文无法访问内部上下文中国的任何东西。上下文之间的连接是线性的、有序的。每个上下文都可以到上一级上下文中去搜索变量和函数，但任何上下文都不能到下一级上下文去搜索。swapColors() 局部上下文的作用链中有 3 个对象：swapColors() 的变量对象、changeColor() 的变量和全局变量对象。swapColors() 的局部上下文首先从自己的变量对象开始搜索变量和函数，搜不到就去搜索上一级变量对象。因此，它不能访问 swapColors() 的上下文。
>注意
>
>函数参数被认为是当前上下文中的变量，因此也跟上下文中的其他变量遵循相同的访问规则。
## ai overview

这段内容系统讲解了 JavaScript 中**执行上下文**与**作用域链**的核心运行机制，下面我帮你拆解核心概念、补充关键细节，并梳理易混淆点，帮你彻底理解这套底层机制。

 一、执行上下文：代码的运行环境

执行上下文是 JS 代码执行的「环境容器」，决定了代码可以访问的数据、this 指向等运行信息。每个执行上下文都对应一个**变量对象（Variable Object, VO）**——当前上下文中所有声明的变量、函数都会被挂载到这个对象上（代码无法直接访问 VO，它是 JS 引擎的内部机制）。

1. 两类核心执行上下文

|类型|创建时机|核心特点|生命周期|
|---|---|---|---|
|全局执行上下文|脚本开始运行时创建，全局唯一|浏览器中对应 `window` 对象；`var` 声明的全局变量会成为 `window` 的属性；`let/const` 顶级声明不会挂载到 `window` 上，但全局范围可访问|页面关闭/程序退出时销毁|
|函数执行上下文|函数被**调用**时创建，调用一次生成一个|函数内部的变量、形参、函数声明存储在对应 VO 中|函数执行完毕后立即销毁|

2. 执行上下文栈（调用栈 Call Stack）

JS 引擎通过**后进先出（LIFO）**的栈结构管理所有执行上下文，控制程序执行流：

1. 脚本启动 → 全局执行上下文入栈（始终位于栈底）
2. 遇到函数调用 → 为该函数创建新的执行上下文，推入栈顶并开始执行函数代码
3. 函数执行结束 → 该函数上下文出栈，控制权交还给栈顶的上一个上下文

> 补充：如果递归调用没有终止条件，会不断创建上下文入栈，超出栈容量就会抛出 `Maximum call stack size exceeded`（栈溢出）错误。

二、变量对象（VO）与活动对象（AO）

二者本质是同一对象的不同阶段称呼，是理解上下文运行的核心细节：

1. **变量对象（VO）**：执行上下文**创建阶段**的称呼，存储当前上下文的变量声明、函数声明、函数形参（此时变量都是未赋值状态，这就是「变量提升」的底层来源）。
2. **活动对象（AO）**：执行上下文**执行阶段**的称呼，仅函数上下文有这个概念。
    - 当函数上下文进入执行阶段，VO 就转为 AO，此时变量开始逐一赋值
    - AO 最初默认包含一个 `arguments` 对象（存储函数实参），全局上下文没有这个对象

 三、作用域链：变量查找的规则

执行上下文在创建时，会基于变量对象生成一条**作用域链（Scope Chain）**，它决定了代码访问变量时的查找顺序。

1. 作用域链的结构

- 作用域链的**最前端**，永远是**当前正在执行的上下文的变量对象（AO/VO）**
- 往后依次是**外层包含上下文**的变量对象，一层一层向上嵌套
- 作用域链的**末端**，永远是**全局上下文的变量对象**
...
1. 先从当前上下文的变量对象中查找
2. 找不到就沿作用域链向上，去父级上下文的变量对象中查找
3. 直到全局上下文的变量对象，仍找不到则抛出 `ReferenceError` 错误

> 关键补充：JS 采用**词法作用域（静态作用域）**——函数的作用域在**函数定义时**就已经确定，和调用位置无关。因此作用域链的层级由代码书写位置决定，不是由调用顺序决定。

四、补充：执行上下文的两个阶段

原文没有明确拆分，这是理解变量提升的核心逻辑：

1. **创建阶段**：函数调用后、代码执行前
    - 创建变量对象（变量声明、函数声明提升，函数声明优先级高于变量）
    - 初始化作用域链
    - 确定 this 指向
2. **执行阶段**：逐行执行代码
    - 给变量赋值，执行函数调用
...
1. **作用域 vs 执行上下文**
    - 作用域是**静态**的：函数定义时就确定了变量的可访问范围
    - 执行上下文是**动态**的：函数每次调用都会生成新的执行上下文
    - 关系：同一个作用域下，可以对应多个执行上下文（比如函数被多次调用）
2. **var vs let/const 的全局声明差异**
    ...
    console.log(window.b); // undefined（let 不挂载到 window）
    console.log(b); // 2（全局作用域仍可正常访问）
    ```
    
    二者都在全局作用域可访问，区别只是是否挂载到全局对象上。
    

需要我再举一个完整的代码示例，一步步演示执行上下文创建、作用域链查找的完整过程吗？
## 4.2.1 作用域链增强
