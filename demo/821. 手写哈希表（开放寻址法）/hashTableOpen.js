...
class HashTableOpen {
  ...
  set(key, value) {
    ...
    while (
      ...
      idx = (idx + 1) % this.size;
    if (this.keys[idx] === undefined || this.deleted[idx]) {
      this.count++;
  ...
  get(key) {
    ...
    while (probes < this.size) {
      ...
      if (!this.deleted[idx] && this.keys[idx] === key) return this.values[idx];
      idx = (idx + 1) % this.size;
      probes++;
    }
    return undefined;
  ...
  delete(key) {
    ...
    let probes = 0;
    while (probes < this.size) {
      if (this.keys[idx] === undefined) return false;
      if (!this.deleted[idx] && this.keys[idx] === key) {
  ...
  resize(newSize) {
    ...
    this.deleted = new Array(newSize).fill(false);
...
ht.delete("age");
console.log('删除后 get("age"):', ht.get("age")); // undefined
