...
class ACAutomaton {
  ...
  insert(word) {
    let cur = this.root;
    for (const c of word) {
      if (!cur.children[c]) cur.children[c] = new ACNode();
      cur = cur.children[c];
    }
  ...
  build() {
    const queue = [];
    this.root.fail = null;
    for (const c in this.root.children) {
      this.root.children[c].fail = this.root;
      queue.push(this.root.children[c]);
    }
    while (queue.length) {
  ...
  search(text) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (cur)
        for (const word of cur.output)
          matches.push({ word, end: i + 1, start: i - word.length + 2 });
    }
    return matches;
  }
}
...
console.log("匹配结果:");
results.forEach((m) =>
  console.log("  " + m.word + " at [" + m.start + ", " + m.end + "]"),
);
