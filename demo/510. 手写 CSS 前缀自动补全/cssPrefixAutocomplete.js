...
function cssPrefixAutocomplete(prop, value) {
  ...
  const baseCamel = /^[A-Z]/.test(prop) || prop.indexOf('-') === -1 ? prop : toCamel(prop);
  const testValue = value == null ? 'inherit' : value;

  for (const prefix of VENDOR_PREFIXES) {
    const camel = prefix === '' ? baseCamel : prefix + baseCamel.charAt(0).toUpperCase() + baseCamel.slice(1);
    if (!(camel in el.style)) continue;
    if (value === undefined) {
      return { prop: camel, propKebab: toKebab(camel), prefix: prefix.toLowerCase() };
    }
    el.style[camel] = '';
...
const supportedCamel = new Set(['transform', 'WebkitTransform', 'transition', 'WebkitTransition', 'color']);
const fakeElStyle = {};
const styleProxy = new Proxy(fakeElStyle, {
  has(t, p) { return supportedCamel.has(p); },
  set(t, p, v) { t[p] = (supportedCamel.has(p) ? v : ''); return true; },
  get(t, p) { return t[p]; },
});
globalThis.document = {
...
console.log('transform (no value):', JSON.stringify(cssPrefixAutocomplete('transform')));
// expected: {"prop":"transform","propKebab":"transform","prefix":""}
console.log('transform with value:', JSON.stringify(cssPrefixAutocomplete('transform', 'rotate(30deg)')));
// expected: {"prop":"transform","propKebab":"transform","prefix":""}
console.log('unknown property:', cssPrefixAutocomplete('definitelyNotAProperty'));
// expected: null
console.log('kebab input -> camel:', JSON.stringify(cssPrefixAutocomplete('transition', 'opacity 1s')));
// expected: {"prop":"transition","propKebab":"transition","prefix":""}

// applyPrefixedStyle helper.
const targetEl = { style: styleProxy };
console.log('applyPrefixedStyle transform:', applyPrefixedStyle(targetEl, 'transform', 'scale(2)'));
// expected: true
console.log('applied value:', targetEl.style.transform); // expected: scale(2)

delete globalThis.document;
console.log('node env returns null:', cssPrefixAutocomplete('anything') === null);
// expected: true
