...
console.log('uniqBy floor:', JSON.stringify(uniqBy([2.1, 1.2, 2.3, 3.4], Math.floor))); // expected: [2.1,1.2,3.4]
console.log('uniqBy property:', JSON.stringify(uniqBy([{ x: 1 }, { x: 2 }, { x: 1 }, { x: 3 }], 'x'))); // expected: [{"x":1},{"x":2},{"x":3}]
console.log('uniqBy function key:', JSON.stringify(uniqBy(['apple', 'avocado', 'banana', 'apricot'], (s) => s[0]))); // expected: ["apple","banana"]
console.log('uniqBy keeps first:', JSON.stringify(uniqBy([{ id: 1, n: 'a' }, { id: 1, n: 'b' }, { id: 2, n: 'c' }], 'id'))); // expected: [{"id":1,"n":"a"},{"id":2,"n":"c"}]
