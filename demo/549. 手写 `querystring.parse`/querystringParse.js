...
// maxKeys=0 表示不限制
console.log(Object.keys(querystringParse('a=1&b=2&c=3', '&', '=', { maxKeys: 0 })).length); // 3

// 自定义解码
console.log(querystringParse('x=A_B', '&', '=', {
...
if (typeof require === 'function') {
  try {
    ...
    const cases = ['foo=bar&baz=qux&baz=quux&corge', 'name=%E5%BC%A0%E4%B8%89&age=18', 'q=hello+world'];
    for (const c of cases) {
      const mine = querystringParse(c);
      const native = qs.parse(c);
      console.log(`compare [${c}]:`, JSON.stringify(mine) === JSON.stringify(native));
    }
  } catch (e) {
    // 跳过
  }
