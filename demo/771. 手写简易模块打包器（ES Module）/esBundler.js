...
function transform(code) {
  ...
  code = code.replace(/export\s+\{([^}]+)\}/g, (_, n) =>
    n
      .split(",")
      .map((s) => "module.exports." + s.trim() + " = " + s.trim() + ";")
      .join("\n"),
  );
  code = code.replace(
    /export\s+(const|let|var)\s+(\w+)/g,
    "$1 $2 = $2; module.exports.$2 = $2;",
  );
  return code;
}
...
function buildGraph(entry, modules = {}) {
  ...
  for (const dep of parseImports(code)) {
    const dp = path.resolve(
      path.dirname(p),
      dep + (path.extname(dep) ? "" : ".js"),
    );
...
function bundle(entry) {
  ...
  const arr = Object.entries(modules)
    .map(([k, m]) => {
...
const mockOutput = Object.entries(mockModules)
  .map(
    ([k, m]) =>
      "  " +
      m.id +
...
console.log("打包输出预览:");
console.log(
  "(function(modules){ var cache={}; function __r(id){...} __r(0); })({",
);
console.log(mockOutput);
console.log("});");
console.log(
  "\n解析 import 正则:",
  /import\s+.*?from\s+['"`](.+?)['"`]/g.source,
);
