...
// 定义各种中间件插件
const authMiddleware = function (data) {
  console.log('[Auth] 验证用户: ' + data.user);
  data.authenticated = true;
  return data;
};

const logMiddleware = function (data) {
  console.log('[Log] 记录请求: ' + data.action);
  return data;
};

const validateMiddleware = function (data) {
  console.log('[Validate] 校验参数');
  data.valid = true;
  return data;
};

const transformMiddleware = function (data) {
...
const app = new App();
app.use(authMiddleware).use(logMiddleware).use(validateMiddleware);

const requestData = { user: '张三', action: 'getProfile' };
const result = app.run(requestData);
...
const result2 = app.run({ user: '李四', action: 'updateProfile' });
...
// 反例：违反开放-封闭原则的支付处理器
const PaymentProcessorBad = function () {};
...
// 新增支付方式：只需注册新策略
const wechatPayStrategy = function (amount) {
  const fee = amount * 0.006;
...
// 定义过滤器插件
const removeNulls = function (data) {
  const filtered = [];
...
const deduplicate = function (data) {
  const seen = {};
  const unique = [];
  for (let i = 0; i < data.length; i++) {
    if (!seen[data[i]]) {
...
filter.addFilter(removeNulls).addFilter(deduplicate).addFilter(sortAscending);

const rawData = [5, null, 3, 2, 5, undefined, 1, 3, 4];
console.log('原始数据: ' + JSON.stringify(rawData));

const filtered = filter.process(rawData);
