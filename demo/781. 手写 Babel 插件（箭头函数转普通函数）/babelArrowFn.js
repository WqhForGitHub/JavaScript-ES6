  ...
  return {
    ...
    visitor: {
      ArrowFunctionExpression(path) {
        ...
        path.replaceWith({ type: 'FunctionExpression', id: null, params: node.params, body, async: node.async, generator: false });
      },
    },
  };
};
...
function transformAST(ast, plugin) {
  ...
  function visit(node, parent, key, index) {
    ...
    if (h) {
      h({ node, replaceWith(n) { if (index !== undefined) parent[key][index] = n; else if (parent) parent[key] = n; } });
    }
    for (const k in node) {
      if (Array.isArray(node[k])) node[k].forEach((c, i) => visit(c, node, k, i));
      else if (node[k] && typeof node[k].type === 'string') visit(node[k], node, k);
    }
  }
...
// ===== 测试 =====
const ast = { type: 'Program', body: [{ type: 'VariableDeclaration', kind: 'const', declarations: [{ type: 'VariableDeclarator', id: { type: 'Identifier', name: 'add' }, init: { type: 'ArrowFunctionExpression', params: [{ type: 'Identifier', name: 'a' }, { type: 'Identifier', name: 'b' }], body: { type: 'BinaryExpression', operator: '+', left: { type: 'Identifier', name: 'a' }, right: { type: 'Identifier', name: 'b' } }, async: false } }] }] };
