...
class MockServer {
  ...
  start(port = 3000) {
    ...
    server.listen(port, () => console.log('[MockServer] Running on port', port));
    return server;
  }
}
...
mock.mock('GET', '/api/slow', () => ({ data: 'slow response' }), { delay: 100 });

// 模拟请求
console.log('=== Mock 服务器演示 ===');

const mockRes = { writeHead: (c, h) => console.log('Status:', c), end: (b) => console.log('Response:', b) };
