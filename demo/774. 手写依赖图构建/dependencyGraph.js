  ...
  build(entryPath) {
    ...
    while (queue.length) {
      ...
      if (!this.nodes.has(cur))
        this.nodes.set(cur, new ModuleNode(this.idCounter++, cur));
      const node = this.nodes.get(cur);
      for (const dep of this.parseDeps(cur)) {
        if (!this.nodes.has(dep))
          this.nodes.set(dep, new ModuleNode(this.idCounter++, dep));
        const dn = this.nodes.get(dep);
        if (!node.deps.includes(dn.id)) {
          node.deps.push(dn.id);
          dn.incoming.push(node.id);
        }
        queue.push(dep);
      }
    }
