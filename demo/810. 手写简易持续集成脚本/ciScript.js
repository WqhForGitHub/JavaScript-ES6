...
class CIScript {
  ...
  async runStage(stage) {
    ...
    if (stage.parallel) {
      const results = await Promise.all(
        stage.steps.map((s) => this.runStep(s)),
      );
    ...
    console.log(
      stageSuccess
        ? "  Stage PASSED (" + duration + "ms)"
        : "  Stage FAILED (" + duration + "ms)",
    );
  ...
  async run() {
    ...
    console.log(
      "\n" +
        (this.report.success ? "✅ Pipeline SUCCESS" : "❌ Pipeline FAILED") +
        " (" +
        totalDuration +
        "ms)",
    );
  ...
  printReport() {
    ...
    console.log(
      "Duration:",
      this.report.endTime - this.report.startTime + "ms",
    );
  }
}
...
ci.stage("Install", [{ name: "npm ci", command: "npm ci" }]);
