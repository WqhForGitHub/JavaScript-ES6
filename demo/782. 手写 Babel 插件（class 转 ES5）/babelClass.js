...
function transformAST(ast, plugin) {
  ...
  function visit(node, parent, key, index) {
    ...
    if (h) {
      h({
        ...
        replaceWithMultiple(nodes) {
          if (index !== undefined) parent[key].splice(index, 1, ...nodes);
        },
      });
    }
    const cur =
      index !== undefined && parent
        ? parent[key][index]
        : parent
          ? parent[key]
          : node;
    if (!cur) return;
    for (const k in cur) {
      if (Array.isArray(cur[k])) cur[k].forEach((c, i) => visit(c, cur, k, i));
      else if (cur[k] && typeof cur[k].type === "string") visit(cur[k], cur, k);
    }
  }
