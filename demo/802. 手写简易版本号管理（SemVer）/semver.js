...
class SemVer {
  ...
  compare(other) {
    ...
    for (
      let i = 0;
      i < Math.max(this.prerelease.length, other.prerelease.length);
      i++
    ) {
    ...
    switch (type) {
      ...
      case "prerelease":
        if (this.prerelease.length) {
          ...
          if (/^\d+$/.test(last))
            this.prerelease[this.prerelease.length - 1] = String(
              parseInt(last) + 1,
            );
  ...
  satisfies(range) {
    ...
    switch (op) {
      ...
      case "~":
        return (
          this.major === target.major && this.minor === target.minor && cmp >= 0
        );
      case "^":
        return this.major === target.major && cmp >= 0;
      default:
        return cmp === 0;
    }
  }
...
console.log(
  "比较 1.2.4-alpha.1 vs 1.2.4-alpha.2:",
  v2.compare(SemVer.parse("1.2.4-alpha.2")),
); // -1
