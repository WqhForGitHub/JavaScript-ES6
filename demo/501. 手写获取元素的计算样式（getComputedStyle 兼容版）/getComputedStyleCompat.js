...
function getComputedStyleCompat(el, prop, pseudoElt) {
  ...
  const camel = /^[A-Z]/.test(prop) || prop.indexOf('-') === -1 ? prop : toCamel(prop);
  const kebab = prop.indexOf('-') !== -1 ? prop : prop.replace(/[A-Z]/g, (m) => '-' + m.toLowerCase());
...
function getComputedStylesCompat(el, props, pseudoElt) {
  ...
  if (typeof window !== 'undefined' && typeof window.getComputedStyle === 'function') {
    const style = window.getComputedStyle(el, pseudoElt || null);
    props.forEach((p) => {
      const camel = /^[A-Z]/.test(p) || p.indexOf('-') === -1 ? p : p.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
      const kebab = p.indexOf('-') !== -1 ? p : p.replace(/[A-Z]/g, (m) => '-' + m.toLowerCase());
...
console.log('getComputedStyleCompat is a function:', typeof getComputedStyleCompat === 'function');
// expected: getComputedStyleCompat is a function: true
console.log('returns empty for null el:', getComputedStyleCompat(null, 'color') === '');
// expected: returns empty for null el: true
console.log('batch helper is a function:', typeof getComputedStylesCompat === 'function');
