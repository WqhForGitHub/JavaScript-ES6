...
function pathRelative(from, to) {
  if (typeof from !== 'string') {
    throw new TypeError('Path must be a string. Received ' + JSON.stringify(from));
  }
  if (typeof to !== 'string') {
    throw new TypeError('Path must be a string. Received ' + JSON.stringify(to));
  }
...
if (typeof require === 'function') {
  try {
    ...
    for (const [f, t] of cases) {
      ...
      console.log(`compare [${f} -> ${t}]: mine=${mine} native=${native} same=${mine === native}`);
    }
  } catch (e) {
    // 跳过
  }
