    ...
    return {
      VariableDeclarator(node) {
        if (node.id.type === "Identifier")
          declared.set(node.id.name, { node, used: false });
      },
      Identifier(node) {
        if (declared.has(node.name)) declared.get(node.name).used = true;
      },
      "Program:exit"() {
        for (const [name, info] of declared)
          if (!info.used)
            ctx.report(info.node, "'" + name + "' is defined but never used.");
      },
    };
  },
};
...
function parseAndRun(code) {
  ...
  allIdents.forEach((name) => {
    if (declared.has(name)) visitors.Identifier({ type: "Identifier", name });
  });
