...
console.log('no-DOM -> false:', isSelfVisible({ nodeType: 1 }) === false || typeof window !== 'undefined');
...
function makeFakeTree() {
  ...
  function styleOf(node) { return styles.get(node) || {}; }
  const fakeWindow = {
    getComputedStyle: (n) => {
      const s = styleOf(n);
...
const parent = makeEl(null, { display: 'block', visibility: 'visible', opacity: '1' });
const child = makeEl(parent, { display: 'block', visibility: 'visible', opacity: '1' });
...
console.log('ancestor visibility:hidden (simplified -> false):', isVisible(visChild)); // expected: false
