...
class DevServer {
  ...
  serveStatic(req, res, pathname) {
    ...
    res.writeHead(200, { 'Content-Type': this.getMime(path.extname(filePath)) });
  ...
  stop() {
    if (this.server) { this.server.close(); console.log('[DevServer] Stopped'); }
  }
}
...
const mockRes1 = {
  writeHead: (code, headers) => console.log('Response:', code, JSON.stringify(headers)),
  setHeader: (k, v) => console.log('Set header:', k, '=', v),
  end: (body) => console.log('Body:', body),
};
// 模拟中间件链
let i = 0;
const next1 = () => { if (i < server.middleware.length) server.middleware[i++](mockReq1, mockRes1, next1); else server.handleRequest(mockReq1, mockRes1, '/'); };
next1();

console.log('\n--- 模拟请求 /api/users ---');
const mockReq2 = { method: 'GET', url: '/api/users' };
const mockRes2 = { writeHead: (c, h) => console.log('Response:', c, JSON.stringify(h)), setHeader: ()=>{}, end: (b) => console.log('Body:', b) };
let j = 0;
const next2 = () => { if (j < server.middleware.length) server.middleware[j++](mockReq2, mockRes2, next2); else server.handleRequest(mockReq2, mockRes2, '/api/users'); };
next2();
