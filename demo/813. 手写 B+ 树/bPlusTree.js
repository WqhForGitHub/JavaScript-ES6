...
class BPlusTree {
  ...
  insert(key, val) {
    ...
    leaf.children.splice(pos, 0, val);
  ...
  splitLeaf(leaf) {
    ...
    newLeaf.children = leaf.children.slice(mid);
    ...
    if (leaf === this.root) {
      ...
      this.insertIntoParent(leaf, upKey, newLeaf);
    }
  }
  insertIntoParent(left, key, right) {
    const parent = this.findParent(this.root, left);
    if (!parent) return;
    let pos = parent.keys.findIndex((k) => k > key);
    if (pos === -1) pos = parent.keys.length;
  ...
  findParent(node, child) {
    if (node.leaf || node.children.includes(child))
      return node.children.includes(child) ? node : null;
    for (const c of node.children) {
      const r = this.findParent(c, child);
      if (r) return r;
    }
    return null;
  }
  rangeQuery(start, end) {
    const leaf = this.findLeaf(this.root, start);
    const result = [];
    let cur = leaf;
    while (cur) {
      for (let i = 0; i < cur.keys.length; i++) {
        if (cur.keys[i] >= start && cur.keys[i] <= end)
          result.push({ key: cur.keys[i], val: cur.children[i] });
        if (cur.keys[i] > end) return result;
      }
      cur = cur.next;
    }
    return result;
