...
function smoothScrollTo(target, x, y, opts = {}) {
  ...
  // Native fast-path.
  if (isWindow && typeof window.scrollTo === 'function' && opts.duration === undefined) {
    let cancelled = false;
  ...
  const cancelRaf = typeof cancelAnimationFrame !== 'undefined' ? cancelAnimationFrame : (id) => clearTimeout(id);
...
console.log('smoothScrollTo is a function:', typeof smoothScrollTo === 'function');
// expected: smoothScrollTo is a function: true
console.log('returns {promise, cancel}:', typeof sc.promise === 'object' && typeof sc.cancel === 'function');
