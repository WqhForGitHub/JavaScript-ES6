...
function matchLayer(layer, url) {
  if (layer.type === 'mw') {
    ...
    if (url === layer.path || url.startsWith(layer.path + '/') || url.startsWith(layer.path)) {
      return { params: {} };
    }
    return null;
  }
...
function express() {
  ...
  function use(...args) {
    let path = '/';
    let fns = args;
    if (typeof args[0] === 'string') { path = args[0]; fns = args.slice(1); }
  ...
  function _route(method) {
    return function (path, ...fns) {
      fns.forEach((fn) => stack.push({ type: 'route', method, path, fn, regex: pathToRegex(path, { end: true }) }));
      return app;
    };
  }
  ...
  function handle(req, res, done) {
    ...
    function next(err) {
      ...
      if (!layer) {
        ...
        if (err) { res.statusCode = 500; res.end = res.end || (() => {}); res.end('Internal Error'); }
        return;
      }
      // Method match for routes.
      if (layer.type === 'route' && layer.method.toUpperCase() !== (req.method || 'GET').toUpperCase()) {
        return next(err);
      }
      ...
      try {
        if (err) {
          ...
          if (layer.fn.length >= 4) next(err); // error handler skipped when no err
  ...
  const app = {
    ...
    patch: _route('patch'),
    handle,
    listen() { /* no-op for tests */ return app; },
  };
  return app;
}
...
function run(app, method, url) {
  ...
  const res = { statusCode: 200, headers: {}, body: undefined,
    set(k, v) { this.headers[k] = v; }, end(b) { this.body = b; } };
  app.handle(req, res);
  return { req, res };
}

const app = express();
app.use((req, res, next) => { req.log = ['mw1']; next(); });
app.use((req, res, next) => { req.log.push('mw2'); next(); });
app.get('/', (req, res) => { res.end('home:' + req.log.join(',')); });
app.get('/users/:id', (req, res) => { res.end('user:' + req.params.id); });
app.use((err, req, res, next) => { res.statusCode = 500; res.end('err:' + err.message); });
