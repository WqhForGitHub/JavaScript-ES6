...
function createStore(reducer, preloadedState) {
  ...
  function dispatch(action) {
    if (typeof action.type === 'undefined') throw new Error('Actions need a type');
...
const enhancedStore = applyMiddleware(thunk, logger, crashReporter)(createStore)(
  counter,
  undefined
);
...
// ---------------- Test cases ----------------
const events = [];
enhancedStore.subscribe((state, action) => events.push(`${action.type}=${state.value}`));
...
const boomStore = applyMiddleware(crashReporter)(createStore)(throwingReducer, undefined);
try {
