...
class TreeShaker {
  ...
  parseExports(code) {
    ...
    while ((m = r1.exec(code)) !== null)
      m[1].split(",").forEach((n) => exports.add(n.trim()));
  ...
  parseImports(code) {
    ...
    while ((m = r2.exec(code)) !== null)
      imports.push({ source: m[2], names: ["default"] });
    return imports;
  }
  ...
  getShakeable() {
    ...
    for (const [p, info] of this.modules) {
      ...
      if (unused.length)
        result.push({ path: p, unused, used: [...info.usedExports] });
    }
    return result;
  }
}
...
shaker.setSources({
  ...
  "./math":
    "export function add(a,b){return a+b} export function sub(a,b){return a-b} export function mul(a,b){return a*b}",
});
shaker.analyze("entry.js");
const shakeable = shaker.getShakeable();
console.log("可摇除的导出:");
shakeable.forEach((s) =>
  console.log("  " + s.path + ": 未使用 [" + s.unused.join(", ") + "]"),
);
// ./math: 未使用 [sub, mul]
