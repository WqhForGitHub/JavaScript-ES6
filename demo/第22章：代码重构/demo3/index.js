...
// ---------- 重构前 ----------
const date = new Date();
const summerStart = new Date(date.getFullYear(), 5, 1); // 6月1日
const summerEnd = new Date(date.getFullYear(), 8, 30); // 9月30日
let charge1 = 100;
const quantity1 = 10;
...
// ---------- 重构后 ----------
// 将条件判断提取为语义清晰的函数
const isSummer = function (aDate) {
  return aDate > summerStart && aDate < summerEnd;
};

const summerRate = 0.8;
...
const ORDER_STATUS_SIGNED = 3;
const ORDER_STATUS_CANCELLED = 4;

const status2 = ORDER_STATUS_SHIPPED;
...
const status3 = ORDER_STATUS.SHIPPED;
