...
const FlyweightModel = function (sex) {
  this.sex = sex;
};
...
const modelManager = (function () {
  ...
  return {
    add: function (id, sex, underwear) {
      const flyweight = modelFactory.create(sex);
      modelDatabase[id] = {
        flyweight: flyweight,
        underwear: underwear,
      };
...
// 验证共享
const maleA = modelFactory.create('male');
const maleB = modelFactory.create('male');
console.log('两次 create("male") 返回同一对象：' + (maleA === maleB));

const femaleA = modelFactory.create('female');
const femaleB = modelFactory.create('female');
