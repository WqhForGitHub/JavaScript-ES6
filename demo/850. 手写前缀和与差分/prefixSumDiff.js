...
class Difference {
  ...
  static fromArray(arr) {
    const d = new Difference(arr.length);
    d.diff[0] = arr[0];
    for (let i = 1; i < arr.length; i++) d.diff[i] = arr[i] - arr[i - 1];
    return d;
  }
  add(l, r, val) {
    this.diff[l] += val;
    if (r + 1 <= this.n) this.diff[r + 1] -= val;
  }
  toArray() {
    const arr = new Array(this.n);
    arr[0] = this.diff[0];
    for (let i = 1; i < this.n; i++) arr[i] = arr[i - 1] + this.diff[i];
    return arr;
  }
}
// 二维前缀和
