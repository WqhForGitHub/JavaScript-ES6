...
console.log(
  "groupBy floor:",
  JSON.stringify(groupBy([6.1, 4.2, 6.3], Math.floor)),
); // expected: {"4":[4.2],"6":[6.1,6.3]}
console.log(
  "groupBy length:",
  JSON.stringify(groupBy(["one", "two", "three"], "length")),
); // expected: {"3":["one","two"],"5":["three"]}
console.log(
  "groupBy property:",
  JSON.stringify(groupBy([{ x: 1 }, { x: 2 }, { x: 1 }], "x")),
); // expected: {"1":[{"x":1},{"x":1}],"2":[{"x":2}]}
console.log(
  "groupBy object:",
  JSON.stringify(groupBy({ a: 1.1, b: 2.4, c: 1.9 }, Math.floor)),
); // expected: {"1":[1.1,1.9],"2":[2.4]}
console.log(
  "groupBy default identity:",
  JSON.stringify(groupBy([1, 2, 1, 3, 2])),
