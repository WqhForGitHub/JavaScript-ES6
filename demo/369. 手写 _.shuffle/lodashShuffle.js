...
console.log('shuffle same elements:', JSON.stringify(sh.slice().sort((a, b) => a - b)) === JSON.stringify(input)); // true
...
console.log('shuffle string length:', strSh.length === 3 && strSh.slice().sort().join('') === 'abc'); // true

// Test 3: shuffle object collection
const objSh = shuffle({ a: 1, b: 2, c: 3 });
console.log('shuffle object multiset:', JSON.stringify(objSh.slice().sort()) === JSON.stringify([1, 2, 3])); // true
