...
```javascript
let a = new Array(5); // 没有元素，a.length 是 5
a = []; // 创建一个空数组，此时 length = 0
a[1000] = 0; // 赋值增加了一个元素，length 变成了 1001
```



```javascript
let a1 = [,]; // 这个数组没有元素，length 是 1
...
a.push("zero"); // 在末尾添加一个值，a 为 ["zero"]
a.push("one", "two"); // 再在末尾添加两个值，a 为 ["zero", "one", "two"]
```
...
**`第一个参数指定要把第一个元素复制到的目的索引。第二个参数指定要复制的第一个元素的索引。如果省略第二个参数，则默认值为 0。第三个参数指定要复制的元素切片的终止索引。如果省略，则使用数组的长度。从起始索引到（但不包含）终止索引的元素会被复制。`**
...
**`includes() 使用稍微不同的相等测试，认为 NaN 与自身相等。这意味着 indexOf() 无法检测到数组中的 NaN 值，但 includes() 可以`**
...
a.sort(); // a 为 ["apple", "banana", "cherry"]
```
...
a.sort(); // a 为 [1111, 222, 33, 4]，字母顺序
a.sort(function(a, b) {
    return a - b;
}); // a 为 [4, 33, 222, 1111]，数值顺序

a.sort((a, b) => b - a); // a 为 [1111, 222, 33, 4]，相反的数值顺序
```
...
a.sort(); // a 为 ["Bug", "Dog", "ant", "cat"]，区分大小写的排序
